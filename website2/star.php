...
<head>
	...
	<script src="jquery.fancybox.min.js"></script></head>
<body>
	...
	<form id="page" method="get" action="star.php"></form>	
	<?php
	include('session.php');
...
if(isset($_GET["tounstar"]))
{
$starid=$_GET["tounstar"];
	$starsql="update big_giftshop.products set star=0 where product_id=$starid";
	$conn->query($starsql);
}
if(isset($_GET["tostar"]))
{
	$starsql="update big_giftshop.products set star=1 where product_id=$starid";
	$conn->query($starsql);
}
	$sqlpage="SELECT count(*) AS pagecount from big_giftshop.products where star=1";
		...
		<?php
		$sql="SELECT * FROM big_giftshop.products where star=1 order by products.category1 asc, products.price asc limit 0,12";
		if(isset($_GET["pagenumber"]))
			{
				$p=$_GET["pagenumber"];
				$sql="SELECT * FROM big_giftshop.products where star=1 order by products.category1 asc, products.price asc limit $p,12";
				
			}
		if (isset($_GET["search"]))
		{
			$keyword=$_GET["search"];
			$sql="SELECT * FROM big_giftshop.products where products.name like '%$keyword%' or products.category1 like '%$keyword%' or products.category2 like '%$keyword%' or products.description like '%$keyword%'";
		}
		...
		if ($result->num_rows > 0)
		{
			...
			while($row = $result->fetch_assoc()) 
			{
				...
				echo "
				<tr>
					<td class='pid'>".$row['product_id'] ."</td>";
