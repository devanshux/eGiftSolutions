	...
	<div id="cat">
		 ...
		 Category Name:
			<div class="mdl-textfield mdl-js-textfield">
				<input class="mdl-textfield__input" type="text" id="sample1" name="newcategory" form="addcat">
				<label class="mdl-textfield__label" for="sample1">Text...</label>
			</div>
			<br>
		<button class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored  mdl-js-ripple-effect" name="categorybutton" value="adda" form="addcat">
		Submit
		</button> 
		...
		if(isset($_POST["newcategory"]))
		{
			...
			$new=trim($new," ");
			$sql="insert into big_giftshop.products_categories(categoryname) values('$new')";
...
<br><br>
<?php
if(!isset($_POST["recategorybutton"]))
{
?>
Select Category to Rename:<br>
<select name="category1" form="renamecat">
			<?php
...
<button class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored  mdl-js-ripple-effect" name="recategorybutton" value="rename" form="renamecat">
		Rename
		</button>
<?php
}
...
$oldcat=$_POST["category1"];
	echo "<br>Enter New Name: <div class='mdl-textfield mdl-js-textfield'>";
	echo "<input class='mdl-textfield__input' type='text' id='sample1' name='renewcatname' value='$oldcat' form='newname'>";
	echo "</div>";

?>
<button class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored  mdl-js-ripple-effect" name="newnamebutton" value="<?php echo $oldcat;?>" form="newname">
		Rename
		</button>
<?php
}
if(isset($_POST["renewcatname"]))
{
$newname=$_POST["renewcatname"];
$oldname=$_POST["newnamebutton"];
$newname=trim($newname," ");
echo "<br>Old Category name: " . $oldname;
echo "<br>New Category name: " . $newname;
$sql="update big_giftshop.products_categories set categoryname='$newname' where categoryname='$oldname'";
$conn->query($sql);
$sqlcheck="select count(*) as count from big_giftshop.products where category1='$oldname'";
$res=$conn->query($sqlcheck);
$resarr=$res->fetch_assoc();
if($resarr["count"]>0)
{
echo "inside if";
$sql2="update big_giftshop.products set category1='$newname' where category1 like '$oldname'";
echo "<br>";
$res = $conn->query($sql2);
}
echo "<br>Category Renamed.";
