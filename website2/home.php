...
<?php
	$sqlpage="SELECT count(*) AS pagecount from big_giftshop.products";
		...
		<?php
		$sql="SELECT * FROM big_giftshop.products order by products.category1 asc , products.price asc limit 0,12";
		...
		if(isset($_GET["pagenumber"]))
			{
				$p=$_GET["pagenumber"];
				$sql="SELECT * FROM big_giftshop.products order by products.category1 asc, products.price asc limit $p,12";
		...
		if ($result->num_rows > 0)
		{
			...
			while($row = $result->fetch_assoc()) 
			{
				...
				<tr>
					...
					$star=$row["star"];
					if($star==1)
						echo "<i class='material-icons star' style='color:#03a9f4;'>star</i>";
					echo "</td>";
					echo "<td><a href='/images/".$row['product_id']."/1.jpg' data-fancybox>";
					echo "<img class='prodimage2' src='/images/".$row['product_id']."/1.jpg'/></a>"; 
					echo "</td>";
					echo "<td class='name'>".$row['name'];
