  ...
  <div class="mdl-layout__drawer">
	...
	<ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect " for="submenu">
		...
		while($categoryarray = $categoryres->fetch_assoc())
		{
			...
			echo "<a href='category.php?categoryname=$currentcategory' class='mdl-menu__item'>$currentcategory</a>";
  ...
  <main class="mdl-layout__content">
	...
	<div class="sortbuttons">
	  ...
	  <li class="mdl-menu__item"><button type="submit" name="sortselect" form="sendsort" class="mdl-menu__item linknostyle" value="ID Ascending" >ID Ascending</button></li>
		...
		if (isset($_POST["addtocart"]))
{
$categorysearch=$_POST["categoryname"];
}
else
{
$categorysearch=$_GET['categoryname'];
		...
		$pagecount=0;
		$sqlpage="SELECT count(*) AS pagecount from big_giftshop.products where products.category1 like '$categorysearch' or products.category2 like '$categorysearch' and hidden=0";
		...
		<div class="mdl-grid">
			...
			$sql="SELECT * FROM big_giftshop.products WHERE (products.category1 LIKE '$categorysearch' or products.category2 like '$categorysearch') and hidden=0 order by products.category1 asc, products.price asc limit 0,12";
			if(isset($_GET["pagenumber"]))
			{
				$p=$_GET["pagenumber"];
				$sql="SELECT * FROM big_giftshop.products where (products.category1 like '$categorysearch' or products.category2 like '$categorysearch') and hidden=0 order by products.category1 asc, products.price asc limit $p,12";
